...
--
-- 表结构：
-- 1. policies - 政策主表，存储政策的元数据和内容
-- 2. tags - 标签表，支持三级标签体系（一级政策类型、二级分类、三级标签）
-- 3. policy_tags - 政策与标签的多对多关联表，支持置信度和来源
-- 4. policy_relations - 政策间的关系表，记录政策的替代、修订、引用等关系
-- 5. processing_logs - 处理日志表，记录文档上传、处理、标签生成等操作
--
-- 设计特点：
-- - 外键关系使用 ON DELETE CASCADE，删除记录时自动清理关联数据
-- - 每个表都建有索引以提升查询性能
-- - 使用 TIMESTAMP 记录创建和更新时间，便于追踪数据变化
-- - 唯一约束防止重复数据
-- =============================================================================
