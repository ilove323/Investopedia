...
def render_documents_list():
    ...
    try:
        ...
        col2.metric("有效文档", len([p for p in policies if p.status == "ACTIVE"]))
        col3.metric("失效文档", len([p for p in policies if p.status == "EXPIRED"]))
        col4.metric("即将失效", len([p for p in policies if p.status == "EXPIRING_SOON"]))
        ...
        for policy in policies:
            ...
            with col_card:
                render_policy_card(
                    policy,
                    show_actions=True,
                    on_click_details=lambda p_id=policy.id: st.session_state.update({"show_detail": p_id}),
                    on_click_delete=lambda p_id=policy.id: delete_policy(p_id),
                    on_click_edit=lambda p_id=policy.id: st.session_state.update({"edit_policy_id": p_id})
                )

            # 显示详情
            if st.session_state.get("show_detail") == policy.id:
                with st.expander(f"📖 {policy.metadata.title} - 详情", expanded=True):
...
def render_manage_section():
    ...
    if filtered_policies:
        policy_to_edit = st.selectbox(
            ...
            format_func=lambda p: f"{p.metadata.title} ({p.status})"
        )

        if policy_to_edit:
            with st.expander(f"✏️ 编辑 {policy_to_edit.metadata.title}", expanded=True):
                render_policy_form(policy_to_edit)

    # 风险文档提示
    st.divider()
    st.subheader("⚠️ 即将失效的文档")

    expiring_policies = [p for p in policies if p.status == "EXPIRING_SOON"]
    if expiring_policies:
        for policy in expiring_policies:
            with st.container(border=True):
                st.warning(f"📅 {policy.metadata.title} 即将于 {policy.metadata.expiration_date} 失效")
    else:
        st.info("无即将失效的文档")
